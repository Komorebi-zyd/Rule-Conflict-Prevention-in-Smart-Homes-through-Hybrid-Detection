...
\subsection{Runtime Dynamic Monitoring and Assertion Verification}
...
\begin{table}[htbp]
	...
	\begin{adjustbox}{width=1.0\textwidth}
		\begin{tabular}{c|c}
			...
			\textbf{Classification} & \textbf{Expression}\\
			\hline
			Trigger-Based Conflict &
			\makecell{
				$obs(T_1), obs(C_1), obs(A_1)$ \\
				$obs(T_2), obs(C_2)$ \\
				$intime(A_1, T_2, \delta)$}\\
			\hline
			Condition-Based Conflict (Make Conditions Forbidden) &
			\makecell{
				$obs(C_2)$ \\
				$obs(T_1), obs(C_1), obs(A_1)$ \\
				$obs(\neg C_2)$ \\
				$intime(A_1, \neg C_2, \delta)$}\\
			\hline
			Condition-Based Conflict (Make Conditions Satisfied) &
			\makecell{
				$obs(\neg C_2)$ \\
				$obs(T_1), obs(C_1), obs(A_1)$ \\
				$obs(C_2)$ \\
