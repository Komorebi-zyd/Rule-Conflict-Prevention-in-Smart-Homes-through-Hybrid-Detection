...
\begin{table*}[t]
	\centering
	\caption{Selected conflict-related rule pairs and detection results. Columns include all manually confirmed conflict cases under the configured Entity Safety Configuration and representative non-conflict rule pairs flagged by at least one system. Each cell indicates whether the corresponding system reports that rule pair under a specific conflict category. Here, $\checkmark$ denotes that the reported pair/category matches a manually confirmed conflict case; $\times$ denotes that the system reports a pair/category that is not a conflict case under the configured deployment and Entity Safety Configuration; and -- denotes that the pair is not reported under that specific category.}
