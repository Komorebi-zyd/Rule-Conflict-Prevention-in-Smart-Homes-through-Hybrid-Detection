...
\begin{table*}[t]
	\centering
	\caption{Conflict detection results for selected rule pairs. $\checkmark$ denotes that the reported pair matches a manually confirmed conflict case; $\times$ denotes that the pair is not a conflict case under current settings; and -- denotes that the pair is not reported under that specific category.}
